const API_URL = "https://like-sand-tuck.glitch.me";
...
const cartCount = document.querySelector(".store__cart-cnt");
...
const fetchCartItems = async (ids) => {
  try {
    ...
    console.error(`Ошибка запросов товаров для корзины: ${error}`);
    return [];
  }
};
...
const sabmitOrder = async (event) => {
  ...
  try {
    ...
    orderMessageText.textContent = `Ваш заказ оформлен. Номер заказа ${orderId}. Вы можете забрать его завтра после 12.00`;
...
cartForm.addEventListener("submit", sabmitOrder);
